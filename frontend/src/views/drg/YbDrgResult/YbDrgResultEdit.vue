...
export default {
  ...
  methods: {
    ...
    setFormValues ({
      ...
      let fields = ['applyDataId', 'verifyId', 'manageId', 'doctorCode', 'doctorName', 'deptCode', 'deptName', 'operateReason', 'operateDate', 'applyDateStr', 'orderNumber', 'orderNum', 'areaType']
      let fieldDates = ['operateDate']
      Object.keys(ybDrgResult).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key)
          let obj = {}
          if (fieldDates.indexOf(key) !== -1) {
            if (ybDrgResult[key] !== '') {
              obj[key] = moment(ybDrgResult[key])
            } else {
              obj[key] = ''
            }
          } else {
            obj[key] = ybDrgResult[key]
          }
          this.form.setFieldsValue(obj)
        }
      })
      this.ybDrgResult.id = ybDrgResult.id
    },
    handleSubmit () {
      this.form.validateFields((err, values) => {
        if (!err) {
          let ybDrgResult = this.form.getFieldsValue()
          ybDrgResult.id = this.ybDrgResult.id
          this.$put('ybDrgResult', {
            ...ybDrgResult
          }).then(() => {
            this.reset()
            this.$emit('success')
