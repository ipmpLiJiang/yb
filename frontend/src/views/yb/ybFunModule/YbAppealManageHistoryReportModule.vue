    ...
    <template>
      ...
      >
        <div class="editable-row-operations">
          <span>
            <a v-show="record.acceptState===6?true:false"
              @click.stop="() => look(record)"
            >查看申诉材料</a>
            <label v-show="record.acceptState!==6?true:false">
              无
            </label>
          </span>
        </div>
...
export default {
  name: 'YbAppealManageHistoryReportModule',
  components: {
    AppealDataModule },
  props: {
    historyVisiable: {
  ...
  methods: {
    ...
    // },
    look (record) {
      record.applyDateStr = this.ybAppealManageHistory.applyDateStr
      record.typeno = this.ybAppealManageHistory.typeno
