 ...
 */
@Slf4j
public class MySqlDB<T extends Serializable> {
    // 根据配置文件里的名称创建连接池
    private static DataSource source = null;
    public static Properties pros = null;
    static {
        pros = new Properties();
        InputStream is = MySqlDB.class.getClassLoader().getResourceAsStream("db_mysql.properties");
        try {
            pros.load(is);
    ...
    public List<T> excuteSqlRS(T t, String sql) throws Exception {
        ...
        if(isErr) {
            throw new Exception("调用DRG接口:" + errMsg);
        }
        return resultList;
    }
